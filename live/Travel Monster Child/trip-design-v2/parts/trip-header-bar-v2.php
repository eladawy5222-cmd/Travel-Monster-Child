 ...
 * ----------------------------------------------------- */
if ( ! function_exists( 'fts_resolve_lang_flag_url' ) ) {
    function fts_resolve_lang_flag_url( $code ) {
        $rel  = '/plugins/sitepress-multilingual-cms/res/flags/';
        $dir  = WP_CONTENT_DIR . $rel;
        $url  = content_url( $rel );
        $code = strtolower( (string) $code );
        $try  = array( $code );
        if ( $code === 'zh-hans' ) {
            $try = array( 'zh-hans', 'zh', 'zh-cn', 'cn' );
        }
        foreach ( $try as $c ) {
            if ( is_readable( $dir . $c . '.svg' ) ) {
                return $url . $c . '.svg';
            }
        }
        return '';
...
if ( ! function_exists( 'fts_get_language_switcher_items' ) ) {
    function fts_get_language_switcher_items() {
            ...
            if ( ! empty( $raw ) && is_array( $raw ) ) {
                foreach ( $raw as $l ) {
                    ...
                    $langs[] = array(
                        ...
                        'flag'   => ! empty( $l['country_flag_url'] ) ? $l['country_flag_url'] : fts_resolve_lang_flag_url( $code ),
        ...
        if ( empty( $langs ) ) {
            $site_base = home_url();
            $langs = array(
                array( 'code' => 'en',      'name' => 'English',  'url' => $site_base . '/',          'flag' => fts_resolve_lang_flag_url( 'en' ),      'active' => true  ),
                array( 'code' => 'ar',      'name' => 'العربية',  'url' => $site_base . '/ar/',       'flag' => fts_resolve_lang_flag_url( 'ar' ),      'active' => false ),
                array( 'code' => 'de',      'name' => 'Deutsch',  'url' => $site_base . '/de/',       'flag' => fts_resolve_lang_flag_url( 'de' ),      'active' => false ),
                array( 'code' => 'es',      'name' => 'Español',  'url' => $site_base . '/es/',       'flag' => fts_resolve_lang_flag_url( 'es' ),      'active' => false ),
                array( 'code' => 'fr',      'name' => 'Français', 'url' => $site_base . '/fr/',       'flag' => fts_resolve_lang_flag_url( 'fr' ),      'active' => false ),
                array( 'code' => 'it',      'name' => 'Italiano', 'url' => $site_base . '/it/',       'flag' => fts_resolve_lang_flag_url( 'it' ),      'active' => false ),
                array( 'code' => 'ro',      'name' => 'Română',   'url' => $site_base . '/ro/',       'flag' => fts_resolve_lang_flag_url( 'ro' ),      'active' => false ),
                array( 'code' => 'ru',      'name' => 'Русский',  'url' => $site_base . '/ru/',       'flag' => fts_resolve_lang_flag_url( 'ru' ),      'active' => false ),
                array( 'code' => 'tr',      'name' => 'Türkçe',   'url' => $site_base . '/tr/',       'flag' => fts_resolve_lang_flag_url( 'tr' ),      'active' => false ),
                array( 'code' => 'ja',      'name' => '日本語',     'url' => $site_base . '/ja/',       'flag' => fts_resolve_lang_flag_url( 'ja' ),      'active' => false ),
                array( 'code' => 'zh-hans', 'name' => '简体中文',   'url' => $site_base . '/zh-hans/',  'flag' => fts_resolve_lang_flag_url( 'zh-hans' ), 'active' => false ),
            );
        }
        ...
        <div class="fts-v2-thb-utils">
            ...
            <div class="fts-v2-thb-item fts-v2-thb-currency">
                ...
                if ( shortcode_exists( 'wte_currency_converter' ) ) {
                    echo do_shortcode( '[wte_currency_converter]' );
                } elseif ( shortcode_exists( 'fts_currency_switcher' ) ) {
            ...
            <div class="fts-v2-thb-item fts-v2-thb-lang">
                <div class="fts-v2-lang-switcher" id="fts-v2-lang-switcher">
                    <button type="button" class="fts-v2-lang-current" id="fts-v2-lang-current" aria-expanded="false" aria-haspopup="menu" aria-controls="fts-v2-lang-dropdown" aria-label="<?php esc_attr_e( 'Language', 'travel-monster-child' ); ?>">
