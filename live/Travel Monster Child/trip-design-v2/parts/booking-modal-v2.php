    ...
    <div class="fts-bm-body" id="fts-bm-body">
      ...
      <div class="fts-bm-step locked" data-step="3" id="fts-bm-step-3">
        ...
        <div class="fts-bm-step-body">
          ...
          <div class="fts-bm-packages-list" id="fts-bm-packages-list">
            <?php
              $bm_min_price = null;
              foreach ( $packages_list as $bm_p ) {
                  $bm_p_dp = floatval( $bm_p['display_price'] ?? 0 );
                  if ( $bm_p_dp > 0 && ( $bm_min_price === null || $bm_p_dp < $bm_min_price ) ) $bm_min_price = $bm_p_dp;
              }
            ?>
            <?php foreach ( $packages_list as $pkg_idx => $pkg ) :
              $card_cls = 'fts-bm-package-card';
              if ( $pkg_idx === 0 ) $card_cls .= ' selected';
              $bm_pkg_dp = floatval( $pkg['display_price'] ?? 0 );
              $bm_is_lowest = ( $bm_min_price !== null && $bm_pkg_dp > 0 && ( $bm_pkg_dp - $bm_min_price ) <= 0.01 );
              if ( $pkg['badge'] === 'most_popular' && ! $bm_is_lowest ) $card_cls .= ' fts-bm-popular';
              if ( $pkg['badge'] === 'best_value' && ! $bm_is_lowest )   $card_cls .= ' fts-bm-best-value';
              if ( $bm_is_lowest ) $card_cls .= ' fts-bm-lowest';
            ?>
            <div class="<?php echo esc_attr( $card_cls ); ?>" data-package-id="<?php echo esc_attr( $pkg['id'] ); ?>">
              <?php if ( $bm_is_lowest ) : ?>
                <div class="fts-bm-badge fts-bm-badge-lowest">&#9660; <?php echo esc_html__( 'LOWEST PRICE', 'fts' ); ?></div>
              ...
              <div class="fts-bm-package-inner">
                ...
                <div class="fts-bm-package-info">
                  ...
                  <?php
                    $bm_pkg_desc = ! empty( $pkg['description_full'] ) ? $pkg['description_full'] : ( ! empty( $pkg['description'] ) ? $pkg['description'] : '' );
                  ?>
                  <?php if ( $bm_pkg_desc !== '' ) : ?>
                    <div class="fts-v2-package-desc-wrap fts-bm-desc-wrap">
                        <p class="fts-bm-package-desc fts-v2-desc-clamped"><?php echo esc_html( $bm_pkg_desc ); ?></p>
      ...
      <div class="fts-bm-step locked" data-step="4" id="fts-bm-step-4">
        ...
        <div class="fts-bm-step-body">
          ...
          <?php if ( ! empty( $pp_eligible ) && is_string( $pay_later_text ) && trim( $pay_later_text ) !== '' ) : ?>
          <div class="fts-bm-payment-plan" id="fts-bm-payment-plan" role="group" aria-label="<?php echo esc_attr__( 'Payment option', 'fts' ); ?>">
            <div class="fts-bm-payment-plan-head">
              <strong><?php echo esc_html__( 'Payment option', 'fts' ); ?></strong>
              <a class="fts-bm-payment-plan-terms" href="<?php echo esc_url( $terms_url ); ?>" target="_blank" rel="noopener"><?php echo esc_html__( 'Terms', 'fts' ); ?></a>
            </div>
            <label class="fts-bm-payment-plan-opt">
              <input type="radio" name="fts_bm_payment_plan" value="deposit" checked>
              <span class="fts-bm-payment-plan-title"><?php echo esc_html__( 'Pay deposit today', 'fts' ); ?></span>
              <span class="fts-bm-payment-plan-desc"><?php echo esc_html( $pay_later_text ); ?></span>
            </label>
            <label class="fts-bm-payment-plan-opt">
              <input type="radio" name="fts_bm_payment_plan" value="full">
              <span class="fts-bm-payment-plan-title"><?php echo esc_html__( 'Pay full amount now', 'fts' ); ?></span>
              <span class="fts-bm-payment-plan-desc"><?php echo esc_html__( 'Pay the full total at checkout.', 'fts' ); ?></span>
            </label>
          </div>
          <?php endif; ?>
