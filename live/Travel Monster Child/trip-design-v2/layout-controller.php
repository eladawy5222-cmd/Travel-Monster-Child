...
if ( ! function_exists( 'wte_get_formated_price' ) ) {
    function wte_get_formated_price( $price ) {
        ...
        $decimals = isset( $settings['decimal_digits'] ) && $settings['decimal_digits'] !== 'default'
            ? intval( $settings['decimal_digits'] )
            : 0;

        return $symbol . number_format( $n, $decimals );
    }
}

if ( ! function_exists( 'fts_v2_get_active_currency_code' ) ) {
    function fts_v2_get_active_currency_code() {
        $from_qs = isset( $_GET['wte_cc'] ) ? (string) $_GET['wte_cc'] : '';
        if ( function_exists( 'wp_unslash' ) ) {
            $from_qs = (string) wp_unslash( $from_qs );
        }
        $from_qs = strtoupper( trim( sanitize_text_field( $from_qs ) ) );
        if ( $from_qs !== '' && preg_match( '/^[A-Z]{3}$/', $from_qs ) ) {
            return $from_qs;
        }
...
if ( ! function_exists( 'fts_v2_benefit_check_svg' ) ) {
    /**
     * Teal circle-check icon (GYG-style trust bullets).
     */
    function fts_v2_benefit_check_svg( $size = 24 ) {
        $size = max( 12, intval( $size ) );
        return '<svg width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke="#007065" stroke-width="1.5"/><path d="M8 12.5 10.5 15 16 9.5" stroke="#007065" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    }
}

if ( ! function_exists( 'fts_v2_resolve_cancellation_desc' ) ) {
    /**
     * Resolve cancellation copy for sidebar benefits (meta, ACF, trip facts, defaults).
     */
    function fts_v2_resolve_cancellation_desc( $trip_id, $free_cancellation_text, $cancel_hours, $trip_facts_items = array(), $at_a_glance = array() ) {
        if ( is_string( $free_cancellation_text ) && trim( $free_cancellation_text ) !== '' ) {
            return trim( $free_cancellation_text );
        }

        if ( $trip_id > 0 && function_exists( 'fts_v2_default_free_cancellation_text' ) ) {
            $default = trim( (string) fts_v2_default_free_cancellation_text( $trip_id ) );
            if ( $default !== '' ) {
                return $default;
            }
        }

        if ( intval( $cancel_hours ) > 0 ) {
            return sprintf(
                /* translators: %s: number of hours */
                __( 'Cancel up to %s hours in advance for a full refund', 'fts' ),
                intval( $cancel_hours )
            );
        }
        ...
        if ( is_array( $trip_facts_items ) ) {
            foreach ( $trip_facts_items as $it ) {
                ...
                if ( strpos( $lbl, 'cancel' ) === false && strpos( $lbl, 'cancellation' ) === false && strpos( $lbl, 'refund' ) === false && strpos( $lbl, 'إلغاء' ) === false ) {
                    continue;
                }
        ...
        if ( is_array( $at_a_glance ) ) {
            foreach ( array( 'cancellation', 'cancel', 'free_cancellation', 'refund' ) as $key ) {
                if ( empty( $at_a_glance[ $key ] ) ) {
                    continue;
                }
                $val = trim( (string) $at_a_glance[ $key ] );
                if ( $val !== '' ) {
                    return $val;
                }
            }
        ...
        $fallback_hours = intval( apply_filters( 'fts_v2_free_cancellation_fallback_hours', 24, $trip_id ) );
        if ( $fallback_hours > 0 ) {
            return sprintf(
                /* translators: %s: number of hours */
                __( 'Cancel up to %s hours in advance for a full refund', 'fts' ),
...
if ( ! function_exists( 'fts_v2_build_booking_benefits' ) ) {
    /**
     * Free cancellation + Reserve now & pay later rows (desktop sidebar under CTA).
     *
     * @return array<int, array{title: string, desc: string, readMore?: bool}>
     */
    function fts_v2_build_booking_benefits( $free_cancellation_text, $cancel_hours, $pp_eligible, $pay_later_text, $terms_url = '', $trip_id = 0, $trip_facts_items = array(), $at_a_glance = array() ) {
        $benefits = array();
        $cancel_desc = fts_v2_resolve_cancellation_desc(
            intval( $trip_id ),
            (string) $free_cancellation_text,
            intval( $cancel_hours ),
            is_array( $trip_facts_items ) ? $trip_facts_items : array(),
            is_array( $at_a_glance ) ? $at_a_glance : array()
        );
        ...
        if ( ! empty( $pp_eligible ) && is_string( $pay_later_text ) && trim( $pay_later_text ) !== '' ) {
            $pl_desc = trim( $pay_later_text );
            if ( strtolower( $pl_desc ) === strtolower( (string) __( 'Reserve now & pay later', 'fts' ) ) ) {
                $pl_desc = '';
            }
            if ( $pl_desc === '' ) {
                $pl_desc = __( 'Keep your travel plans flexible — book your spot and pay nothing today.', 'fts' );
            }
            $row = array(
                'title' => __( 'Reserve now & pay later', 'fts' ),
                'desc'  => $pl_desc,
            );
            if ( is_string( $terms_url ) && trim( $terms_url ) !== '' ) {
                $row['readMore'] = true;
            }
            $benefits[] = $row;
        }
...
class FTS_Trip_Redesign_V2 {
    ...
    public static function cleanup_and_setup() {
        ...
        if ( ! is_admin() && ! wp_doing_ajax() ) {
            $qs_cc = isset( $_GET['wte_cc'] ) ? (string) $_GET['wte_cc'] : '';
            $qs_cc = strtoupper( trim( sanitize_text_field( wp_unslash( $qs_cc ) ) ) );
            if ( $qs_cc !== '' && preg_match( '/^[A-Z]{3}$/', $qs_cc ) ) {
                setcookie( 'cc_code', $qs_cc, time() + 86400 * 30, '/', '', is_ssl(), true );
                setcookie( 'wte_currency_code', $qs_cc, time() + 86400 * 30, '/', '', is_ssl(), true );

                $url = remove_query_arg( 'wte_cc' );
    ...
    public static function get_trip_data() {
        ...
        // On local: serve warm transient to skip heavy DB/processing on repeat loads.
        // TTL is short (90s) so edits are reflected quickly during development.
        $use_transient = ( defined( 'WP_ENVIRONMENT_TYPE' ) && WP_ENVIRONMENT_TYPE === 'local' && $trip_id );
        $trans_key     = 'fts_v2_td_' . (int) $trip_id;
        if ( $use_transient ) {
            $cached = get_transient( $trans_key );
            if ( is_array( $cached ) && ! empty( $cached ) ) {
                // trip_obj is excluded from the transient (not safely serializable);
                // restore it as null so templates relying on it degrade gracefully.
                $cached['trip_obj'] = null;
                self::$trip_data = $cached;
        ...
        $pay_later_text = (string) apply_filters( 'fts_v2_pay_later_text', $pay_later_text, $trip_id, $settings );
        $force_pp = (bool) apply_filters( 'fts_v2_force_partial_payment', true, $trip_id, $settings );
        if ( $force_pp ) {
            $pp_enabled = true;
        }
        $pp_eligible = ( $pp_enabled && is_string( $pay_later_text ) && trim( $pay_later_text ) !== '' );
        ...
        self::$trip_data = compact(
            ...
            'sidebar_trust_items',
            'pp_enabled', 'pay_later_text', 'pp_eligible'
        );

        // On local: cache the rendered trip data for 90s to speed up repeat loads.
        // trip_obj (WTE model object) is excluded because PHP objects are not safe to
        // serialize across requests; it will be re-instantiated on transient hits above.
        if ( $use_transient ) {
            $cacheable = self::$trip_data;
            unset( $cacheable['trip_obj'] );
            set_transient( $trans_key, $cacheable, 90 );
        }
    ...
    public static function render_nuclear_custom_layout() {
        ...
        wp_localize_script( 'fts-trip-v2-script', 'ftsV2Data', array(
            ...
            'payLater'       => array(
                ...
                'text'        => (string) ( $data['pay_later_text'] ?? '' ),
                'termsUrl'    => esc_url_raw( $data['terms_url'] ?? home_url( '/terms-and-conditions/' ) ),
                'defaultPlan' => 'deposit',
            ),
            'i18n'           => array(
                ...
                'read_more'         => __( 'Read more', 'fts' ),
                'book_now'          => __( 'Book Now', 'fts' ),
                'check_availability'=> __( 'Check availability', 'fts' ),
                'pay_deposit'       => __( 'Pay deposit today', 'fts' ),
                'pay_full'          => __( 'Pay full amount now', 'fts' ),
                ...
                'show_more'         => __( 'Show more', 'fts' ),
                'show_less'         => __( 'Show less', 'fts' ),
    ...
    public static function enqueue_assets() {
        try {
            ...
            wp_add_inline_style( 'fts-v2-sticky-critical',
                'body.single-trip{overflow-x:clip!important;overflow-y:visible!important}' .
                ...
                'body.single-trip .post-inner{overflow:visible!important}' .
                ...
                'body.single-trip .fts-v2-main-layout{overflow:visible!important}' .
                'body.single-trip .fts-v2-sidebar-col{overflow:visible!important}' .
                'body.single-trip .entry-content>*:not(.fts-v2-root){overflow:visible!important}' .
                '.fts-v2-tabs-nav{position:sticky!important;top:56px!important;z-index:1000!important;display:none!important}' .
                '@media(max-width:992px){.fts-v2-tabs-nav{display:block!important;margin-left:calc(-1 * var(--fts-container-pad,16px))!important;margin-right:calc(-1 * var(--fts-container-pad,16px))!important;width:calc(100% + 2 * var(--fts-container-pad,16px))!important;box-sizing:border-box!important}}' .
                ...
                '@media(max-width:992px){.fts-v2-gallery-slider{display:block!important}.fts-v2-gallery-grid{display:none!important}.fts-v2-gallery-dots{display:flex!important}}' .
                ...
                '#fts-booking-modal .fts-bm-package-desc{font-size:14px!important;line-height:1.45!important;color:#64748b!important;margin:0!important;display:-webkit-box!important;-webkit-line-clamp:3!important;-webkit-box-orient:vertical!important;overflow:hidden!important}' .
                '#fts-booking-modal .fts-bm-desc-wrap.expanded .fts-bm-package-desc{display:block!important;-webkit-line-clamp:unset!important;overflow:visible!important}' .
