...
(function() {
    ...
    function ftsV2Boot($) {
        ...
        var mobCheckAvailability = ftsT('check_availability', 'Check availability');
        var payDepositText = ftsT('pay_deposit', 'Pay deposit today');
        var payFullText = ftsT('pay_full', 'Pay full amount now');
        ...
        function setCookie(name, value, maxAgeSec) {
            ...
            var m = (typeof maxAgeSec === 'number' && maxAgeSec > 0) ? ('; max-age=' + Math.floor(maxAgeSec)) : '';
            document.cookie = name + '=' + v + '; path=/' + m + '; samesite=lax';
        }

        var bmPaymentPlan = 'full';
        (function initPaymentPlan() {
            var enabled = !!(payLater && payLater.enabled);
            if (!enabled) {
                bmPaymentPlan = 'full';
                return;
            }
            var c = getCookie('ftsPaymentPlan');
            if (c === 'deposit' || c === 'full') {
                bmPaymentPlan = c;
                return;
            }
            var def = payLater && typeof payLater.defaultPlan === 'string' ? payLater.defaultPlan : '';
            bmPaymentPlan = (def === 'deposit') ? 'deposit' : 'full';
        })();
                ...
                var barHtml = '<div class="fts-v2-mobile-book-bar" data-mode="' + (minPkg ? 'from' : 'price') + '">' +
                    ...
                    '<div class="fts-v2-mob-actions">' +
                        (waLink ? '<a href="' + waLink + '" target="_blank" rel="noopener noreferrer nofollow" class="fts-v2-mob-wa" aria-label="WhatsApp">' +
                            ...
                            '</svg>' +
                        '</a>' : '') +
                        '<a href=\"#\" class=\"fts-v2-mob-btn fts-bm-trigger\" data-bm-focus=\"date\">' + mobCheckAvailability + '</a>' +
                    '</div>' +
                '</div>';
        ...
        function ensureBookingModalFallbackStyles() {
            ...
            var css = '' +
                ...
                '#fts-booking-modal .fts-bm-package-desc{font-size:12px!important;color:#64748b!important;margin:0!important;line-height:1.45!important;display:-webkit-box!important;-webkit-line-clamp:3!important;-webkit-box-orient:vertical!important;overflow:hidden!important}' +
                ...
                '#fts-booking-modal .fts-bm-traveler-price{display:flex!important;align-items:center!important;gap:4px!important;margin-top:2px!important}' +
                '#fts-booking-modal .fts-bm-traveler-old-price{font-size:11px!important;color:#94a3b8!important;text-decoration:line-through!important}' +
                '#fts-booking-modal .fts-bm-traveler-current-price{font-size:13px!important;font-weight:700!important;color:#1a2332!important}' +
        ...
        var $tabsScroll = $tabsNav.find('.fts-v2-tabs-scroll');

        function scrollActiveTabIntoView($activeLink) {
            if (!$activeLink.length || !$tabsScroll.length) return;
            var container = $tabsScroll[0];
            var link = $activeLink[0];
        ...
        function updateActiveTab() {
            ...
            var $prev = $tabLinks.filter('.active');
            var prevId = $prev.data('section') || '';
            $tabLinks.removeClass('active');
            if (current) {
                var $active = $tabLinks.filter('[data-section="' + current + '"]');
                $active.addClass('active');
                if (current !== prevId) scrollActiveTabIntoView($active);
            }
        }

        var $pricingSection = $('#fts-v2-sec-pricing');

        function updateMobileBarVisibility() {
            if (!$pricingSection.length) return;
            var $bar = $('.fts-v2-mobile-book-bar');
            if (!$bar.length) return;
            var secTop = $pricingSection.offset().top;
            var secBottom = secTop + $pricingSection.outerHeight();
            var winBottom = $(window).scrollTop() + $(window).height();
            var isVisible = (winBottom > secTop + 80 && $(window).scrollTop() < secBottom);
            if (isVisible) {
                $bar.addClass('fts-v2-mob-bar-hidden');
            } else {
                $bar.removeClass('fts-v2-mob-bar-hidden');
            }
        }
        ...
        $tabLinks.on('click', function(e) {
            ...
            var $accordionItem = target.closest('.fts-v2-accordion-item');
            if (!$accordionItem.length) $accordionItem = target.filter('.fts-v2-accordion-item');

            if ($accordionItem.length && !$accordionItem.hasClass('is-open')) {
                var $accordion = $accordionItem.closest('.fts-v2-accordion');
                if ($accordion.data('single-open')) {
                    $accordion.find('.fts-v2-accordion-item.is-open').not($accordionItem)
                        .removeClass('is-open is-expanded')
                        .find('.fts-v2-accordion-show-more').removeClass('is-visible')
                        .each(function(){ $(this).text($(this).data('more')); });
                }
                $accordionItem.addClass('is-open is-expanded');
            }
        ...
        $(document).on('click', '.fts-v2-hgg-cell[data-index]', function(e) {
            if ($(e.target).closest('.fts-v2-video-play').length) return;
            if ($(e.target).closest('.fts-v2-hgg-viewall').length) return;
            openLightbox(parseInt($(this).data('index'), 10));
        });

        $(document).on('click', '.fts-v2-hero-gallery-viewall[data-action="lightbox"]', function(e) {
            e.preventDefault();
            e.stopPropagation();
            openLightbox(0);
        });

        var ftsGallerySuppressClick = false;

        $(document).on('click', '.fts-v2-gallery-slider .fts-v2-gallery-slide[data-index]', function(e) {
            if (ftsGallerySuppressClick) {
                ftsGallerySuppressClick = false;
                e.preventDefault();
                e.stopPropagation();
                return;
            }
        ...
        (function initGallerySlider() {
            var $slider = $('.fts-v2-hero-gallery .fts-v2-gallery-slider').first();
            if (!$slider.length) {
                $slider = $('.fts-v2-gallery-slider').first();
            }
            ...
            function measure() {
                ...
                slideW = $slider[0].clientWidth || $slider.width() || 0;
            }

            function apply(noAnim, dragOffset) {
                if (noAnim) $track.css('transition', 'none');
                else $track.css('transition', transitionCss);

                var offsetPx = typeof dragOffset === 'number' ? dragOffset : 0;
                if (slideW > 0) {
                    $track.css('transform', 'translate3d(' + (-idx * slideW + offsetPx) + 'px,0,0)');
                } else {
                    var pct = -idx * 100 + (offsetPx && slideW ? (offsetPx / slideW) * 100 : 0);
                    $track.css('transform', 'translate3d(' + pct + '%,0,0)');
                }
            ...
            function startAuto() {
                stopAuto();
                if (!sliderIsVisible() || !mqMobile.matches) return;
            ...
            if (sliderEl) {
                sliderEl.addEventListener('touchstart', function(e) { onStart(e); }, { passive: true });
                sliderEl.addEventListener('touchmove', function(e) { onMove(e); }, { passive: false });
                sliderEl.addEventListener('touchend', function(e) { onEnd(e); }, { passive: true });
                sliderEl.addEventListener('touchcancel', function(e) { onEnd(e); }, { passive: true });
            }
            ...
            function refreshLayout() {
                measure();
                apply(true);
                if (sliderIsVisible() && mqMobile.matches) {
                    startAuto();
                } else {
                    stopAuto();
                }
            ...
            if (typeof ResizeObserver !== 'undefined' && $slider[0]) {
                var ro = new ResizeObserver(function() {
                    if (resizeTimer) clearTimeout(resizeTimer);
                    resizeTimer = setTimeout(refreshLayout, 60);
                });
                ro.observe($slider[0]);
            }
        })();
        ...
        /* ══════════════════════════════════════════════
           Section Accordion (single-open) + Show More
           ══════════════════════════════════════════════ */
        var COLLAPSE_HEIGHT = 300;

        $('.fts-v2-accordion-item').each(function() {
            var $item = $(this);
            var $body = $item.find('.fts-v2-accordion-body');
            var sectionLabel = $item.find('.fts-v2-section-title').text().trim();
            $body.append('<div class="fts-v2-accordion-body-fade"></div>');
            var btnText = sectionLabel ? 'Show full ' + sectionLabel : 'Show more';
            $body.after('<button type="button" class="fts-v2-accordion-show-more" data-more="' + btnText + '" data-less="Show less">' + btnText + '</button>');
        });

        function checkShowMore($item) {
            var $body = $item.find('.fts-v2-accordion-body');
            var $btn = $item.find('.fts-v2-accordion-show-more');
            if (!$item.hasClass('is-open')) { $btn.removeClass('is-visible'); return; }
            var scrollH = $body[0].scrollHeight;
            if (scrollH > COLLAPSE_HEIGHT + 40) {
        ...
        $('.fts-v2-accordion-show-more').on('click', function() {
            var $btn = $(this);
            var $item = $btn.closest('.fts-v2-accordion-item');
            ...
            if (!isExpanded) {
                ...
                var offset = $item.offset().top - 120;
                $('html, body').animate({ scrollTop: offset }, 250);
            }
        });

        $('.fts-v2-accordion-header').on('click', function() {
            var $item = $(this).closest('.fts-v2-accordion-item');
            var $accordion = $item.closest('.fts-v2-accordion');
            var singleOpen = $accordion.data('single-open');
            var isOpen = $item.hasClass('is-open');

            if (singleOpen) {
                $accordion.find('.fts-v2-accordion-item.is-open').not($item).removeClass('is-open is-expanded').find('.fts-v2-accordion-show-more').removeClass('is-visible').each(function(){ $(this).text($(this).data('more')); });
            }

            if (!isOpen) {
                $item.removeClass('is-expanded');
                $item.addClass('is-open');
                $item.find('.fts-v2-accordion-show-more').text($item.find('.fts-v2-accordion-show-more').data('more'));
                setTimeout(function(){ checkShowMore($item); }, 50);
                var offset = $item.offset().top - 120;
                $('html, body').animate({ scrollTop: offset }, 300);
            } else {
                $item.removeClass('is-open is-expanded');
                $item.find('.fts-v2-accordion-show-more').removeClass('is-visible');
            }
        });

        $('.fts-v2-accordion-item.is-open').each(function(){ checkShowMore($(this)); });

        /* ══════════════════════════════════════════════
           Itinerary Accordion (single-open)
           ══════════════════════════════════════════════ */
        $('.fts-v2-tl-header').on('click', function() {
            var $item = $(this).closest('.fts-v2-tl-item');
            var $desc = $item.find('.fts-v2-tl-desc');
            if (!$desc.length) return;

            var isActive = $item.hasClass('active');

            $item.siblings('.fts-v2-tl-item.active')
                .removeClass('active')
                .find('.fts-v2-tl-desc').slideUp(250);
        ...
        /* ══════════════════════════════════════════════
           Map Location Chips
           ══════════════════════════════════════════════ */
        $('#fts-v2-map-chips').on('click', '.fts-v2-map-chip', function() {
            var $chip = $(this);
            var query = $chip.data('query');
            if (!query) return;
            $chip.closest('.fts-v2-map-chips-list').find('.active').removeClass('active');
            $chip.addClass('active');
            var $iframe = $('#fts-v2-map-iframe');
            if ($iframe.length) {
                $iframe.attr('src', 'https://www.google.com/maps?q=' + encodeURIComponent(query) + '&output=embed');
            }
            var $link = $('#fts-v2-map-gmaps-link');
            if ($link.length) {
                $link.attr('href', 'https://www.google.com/maps?q=' + encodeURIComponent(query));
            }
        });
        ...
        function updateTravSummary() {
            ...
            updateSidebarBreakdown();
        }

        function updateSidebarBreakdown() {
            var $bd = $('#fts-v2-sidebar-breakdown');
            if (!$bd.length) return;
            var pkgs = data.packages || [];
            if (!pkgs.length) { $bd.hide(); return; }
            var pkg = pkgs[0];
            try { if (window.ftsV2SelectedPackageId) { for (var i = 0; i < pkgs.length; i++) { if (String(pkgs[i].id) === String(window.ftsV2SelectedPackageId)) { pkg = pkgs[i]; break; } } } } catch(e){}
            var cats = pkg.categories || [];
            if (!cats.length) { $bd.hide(); return; }
            var html = '';
            for (var i = 0; i < cats.length; i++) {
                var c = cats[i];
                var role = String(c.role || '').toLowerCase();
                var label = c.label || '';
                if (role === 'adult') label = adultSing || label || 'Adult';
                else if (role === 'child') label = c.label || childSing || 'Child';
                var count = 0;
                if (role === 'adult') count = sidebarAdults;
                else if (role === 'child') count = sidebarChildren;
                else count = Math.max(c.min_pax || 0, 0);
                if (count <= 0) continue;
                var price = parseFloat(c.display_price) || 0;
                html += '<div class="fts-v2-sb-breakdown-line">' +
                    '<span class="fts-v2-sb-breakdown-label">' + count + ' ' + label + '</span>' +
                    '<span class="fts-v2-sb-breakdown-amount">\u00d7 ' + fmtPriceCompact(price) + '</span>' +
                    '</div>';
            }
            if (html) { $bd.html(html).show(); } else { $bd.hide(); }
        }
        ...
        if ($bm.length) {
            ...
            function bmRenderTravelers(cats) {
                ...
                for (var i = 0; i < cats.length; i++) {
                    ...
                    var priceHtml = '';
                    if (c.display_price > 0) {
                        priceHtml = '<div class="fts-bm-traveler-price">';
                        if (c.has_sale && c.price > c.display_price) {
                            priceHtml += '<span class="fts-bm-traveler-old-price">' + fmtPrice(c.price) + '</span> ';
                        }
                        priceHtml += '<span class="fts-bm-traveler-current-price">' + fmtPrice(c.display_price) + '</span>';
                        priceHtml += '</div>';
                    }
            ...
            function bmSelectPkg(pkgId, skipRender) {
                ...
                if (typeof updateSidebarBreakdown === 'function') updateSidebarBreakdown();
            ...
            function updateStep2Btn() {
                ...
                var $left = $('#fts-bm-traveler-summary .fts-bm-selection-summary-left');
                $left.find('.fts-bm-breakdown-lines').remove();
                if (selectedPkg && tc > 0) {
                    var cats = selectedPkg.categories;
                    var bhtml = '';
                    for (var i = 0; i < cats.length; i++) {
                        var cnt = tCounts[cats[i].id] || 0;
                        if (cnt <= 0) continue;
                        var dp = parseFloat(cats[i].display_price) || 0;
                        bhtml += '<span class="fts-bm-breakdown-item">' + cnt + ' ' + cats[i].label + ' &times; ' + fmtPrice(dp) + '</span>';
                    }
                    if (bhtml) $left.append('<div class="fts-bm-breakdown-lines">' + bhtml + '</div>');
                }
            ...
            function bmOpen() {
                ...
                try {
                    var $pp = $('input[name="fts_bm_payment_plan"]');
                    if ($pp.length && (bmPaymentPlan === 'deposit' || bmPaymentPlan === 'full')) {
                        $pp.filter('[value="' + bmPaymentPlan + '"]').prop('checked', true);
                    }
                } catch (e) {}
                ...
                if (window.requestAnimationFrame) {
                    requestAnimationFrame(function() {
                        ...
                        setTimeout(bmApplyPendingFocus, 120);
                        setTimeout(function() { if (window.ftsCheckDescOverflow) window.ftsCheckDescOverflow(); }, 150);
                    });
                } else {
                    setTimeout(bmOpenHeavy, 0);
                    setTimeout(bmApplyPendingFocus, 120);
            ...
            var bmPendingFocus = '';
            function bmApplyPendingFocus() {
                if (!bmPendingFocus) return;
                var focus = String(bmPendingFocus);
                bmPendingFocus = '';

                if (!$bm || !$bm.length || !$bm.hasClass('active')) return;

                try {
                    if (typeof goToStep === 'function') {
                        if (focus === 'travelers') {
                            var $step2 = $('#fts-bm-step-2');
                            if ($step2.length && !$step2.hasClass('locked')) goToStep(2);
                            else goToStep(1);
                        } else {
                            goToStep(1);
                        }
                ...
                if (bodyEl) {
                    if (focus === 'travelers') {
                        var s2 = document.getElementById('fts-bm-step-2');
                        if (s2 && !s2.classList.contains('locked')) bodyEl.scrollTop = s2.offsetTop - 12;
                ...
                var stepEl = null;
                if (focus === 'travelers') {
                    var ss2 = document.getElementById('fts-bm-step-2');
                    if (ss2 && !ss2.classList.contains('locked')) stepEl = ss2;
                }
                if (!stepEl) stepEl = document.getElementById('fts-bm-step-1');
                if (stepEl) {
                    stepEl.classList.add('fts-bm-focus');
                    setTimeout(function() { stepEl.classList.remove('fts-bm-focus'); }, 900);
                }
            }
            ...
            $(document).on('click', '.fts-bm-trigger', function(e) {
                e.preventDefault();
                e.stopPropagation();
                var focus = $(this).attr('data-bm-focus');
                if (typeof focus === 'string' && focus) bmPendingFocus = focus;
            ...
            $(document).on('change', 'input[name="fts_bm_payment_plan"]', function() {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        if (resp.success && resp.data && resp.data.redirect) {
                            ...
                            if (payLater && payLater.enabled && (bmPaymentPlan === 'deposit' || bmPaymentPlan === 'full')) {
                                setCookie('ftsPaymentPlan', bmPaymentPlan, 86400);
                            }
        ...
        /* ══════════════════════════════════════════════
           Reviews — Topic Filters
           ══════════════════════════════════════════════ */
        $('.fts-v2-reviews-items[data-limit]').each(function() {
            var $wrap = $(this);
            var $filters = $wrap.find('.fts-v2-review-filter[data-topic]');
            ...
            var $status = $wrap.find('.fts-v2-review-filter-status').first();
            var $actions = $wrap.find('.fts-v2-reviews-actions').first();
            var $toggle = $wrap.find('.fts-v2-reviews-toggle').first();

            function setFiltersActive(topic) {
                $filters.each(function() {
                    var $b = $(this);
                    var t = String($b.attr('data-topic') || '');
                    var on = t === topic;
            ...
            function applyFilter(topic) {
                topic = String(topic || 'all');
                setFiltersActive(topic);

                if (topic === 'all') {
                    $items.removeClass('is-filter-hidden');
                    $items.each(function(i) {
                        $(this).toggleClass('is-hidden', i >= limit);
                    });
                    if ($actions.length) $actions.toggle($items.length > limit);
                    if ($toggle.length) {
                        var more = $toggle.attr('data-more') || 'View all reviews';
                        $toggle.attr('data-state', 'collapsed');
                        $toggle.text(more);
                    }
                ...
                $items.each(function() {
                    var $it = $(this);
                    var topics = String($it.attr('data-topics') || '').split(/\s+/).filter(Boolean);
                    var match = false;
                    for (var i = 0; i < topics.length; i++) {
                        if (topics[i] === topic) { match = true; break; }
                    }
                    $it.toggleClass('is-filter-hidden', !match);
                    if (match) {
                        $it.removeClass('is-hidden');
                        shown++;
                    } else {
                        $it.addClass('is-hidden');
                    }
                });

                if ($actions.length) $actions.hide();
                if ($status.length) $status.text(shown ? (shown + ' / ' + $items.length) : ('0 / ' + $items.length));
                ftsTrack('reviews_filter', { topic: topic, shown: shown, total: $items.length });
            }

            $wrap.on('click', '.fts-v2-review-filter[data-topic]', function(e) {
                e.preventDefault();
                var topic = String($(this).attr('data-topic') || 'all');
                applyFilter(topic);
            });
    ...
    /* ─── Package description show more / show less ─── */
    function ftsCheckDescOverflow() {
        var wraps = document.querySelectorAll('.fts-v2-package-desc-wrap');
        wraps.forEach(function(wrap) {
            var desc = wrap.querySelector('.fts-v2-desc-clamped');
            var btn = wrap.querySelector('.fts-v2-desc-toggle');
            if (!desc || !btn) return;
            var dShort = desc.getAttribute('data-short') || '';
            var dFull = desc.getAttribute('data-full') || '';
            if (dFull && dShort && dFull !== dShort) {
                btn.style.display = '';
                return;
            }
            if (desc.scrollHeight > desc.clientHeight + 1) {
                btn.style.display = '';
            } else if (!wrap.classList.contains('expanded')) {
                btn.style.display = 'none';
            }
        });
    }
    window.ftsCheckDescOverflow = ftsCheckDescOverflow;
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', ftsCheckDescOverflow);
    } else {
        ftsCheckDescOverflow();
    }
    window.addEventListener('resize', ftsCheckDescOverflow);

    document.addEventListener('click', function(e) {
        var btn = e.target.closest('.fts-v2-desc-toggle');
        if (!btn) return;
        var wrap = btn.closest('.fts-v2-package-desc-wrap');
        if (!wrap) return;
        ...
        if (desc) {
            var dShort = desc.getAttribute('data-short') || '';
            var dFull = desc.getAttribute('data-full') || '';
            if (dShort && dFull && dShort !== dFull) {
                desc.textContent = isExpanded ? dFull : dShort;
            }
        }
        var more = btn.getAttribute('data-more') || (typeof ftsV2Data !== 'undefined' && ftsV2Data.i18n && ftsV2Data.i18n.show_more ? ftsV2Data.i18n.show_more : 'Show more');
        var less = btn.getAttribute('data-less') || (typeof ftsV2Data !== 'undefined' && ftsV2Data.i18n && ftsV2Data.i18n.show_less ? ftsV2Data.i18n.show_less : 'Show less');
...
(function() {
    ...
    async function ftsShareTrip() {
        ...
        if (!ftsCopyFallback(url)) {
            try { window.prompt('Copy link:', url); } catch (e) {}
        }
    }

    window.ftsV2ShareTrip = ftsShareTrip;

    document.addEventListener('click', function(e) {
        var t = e.target;
        if (!t) return;
        var btn = t.closest ? t.closest('[data-fts-share="trip"]') : null;
        if (!btn) return;
        e.preventDefault();
        ftsShareTrip();
    }, true);
