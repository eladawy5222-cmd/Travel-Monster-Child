...
add_action('wp_footer', function() {
    ...
    if ($v_url) {
        ...
        elseif (strpos($v_url, 'vimeo')!==false) { 
            ...
            $vimeo_id = $m[1] ?? '';
            $embed='https://player.vimeo.com/video/'.$vimeo_id.'?autoplay=1';
            if ( $vimeo_id ) {
                $cache_key = 'fts_vimeo_oembed_' . md5( $vimeo_id . '|' . $v_url );
                $cached    = get_transient( $cache_key );
                if ( is_array( $cached ) && ! empty( $cached['thumbnail'] ) ) {
                    $thumb = (string) $cached['thumbnail'];
                } else {
                    $oembed_url = add_query_arg( array( 'url' => $v_url ), 'https://vimeo.com/api/oembed.json' );
                    $data = function_exists( 'fts_external_get_json' )
                        ? fts_external_get_json( $oembed_url, array( 'timeout' => 6 ) )
                        : null;
                    if ( $data && ! is_wp_error( $data ) && is_object( $data ) && ! empty( $data->thumbnail_url ) ) {
                        $thumb = esc_url_raw( (string) $data->thumbnail_url );
                        set_transient( $cache_key, array( 'thumbnail' => $thumb ), 12 * HOUR_IN_SECONDS );
                    } else {
                        set_transient( $cache_key, array( 'thumbnail' => '' ), HOUR_IN_SECONDS );
                    }
                }
            }
        }
