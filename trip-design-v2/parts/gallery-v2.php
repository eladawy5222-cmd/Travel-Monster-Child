...
<div class="fts-v2-hero-gallery<?php echo $hgg_show_grid ? '' : ' fts-v2-hero-gallery--slider-only'; ?>" data-count="<?php echo esc_attr( $hgg_total ); ?>">
    ...
    <div class="fts-v2-hero-actions">
        <button type="button" class="fts-v2-hero-action" aria-label="<?php echo esc_attr__( 'Share', 'fts' ); ?>" data-fts-share="trip">
            <?php
                $fts_share_svg_path = get_stylesheet_directory() . '/trip-design-v2/assets/svg/share-icon-old-nodes.svg';
                $fts_share_svg = '';
                if ( file_exists( $fts_share_svg_path ) ) {
                    $fts_share_svg = (string) file_get_contents( $fts_share_svg_path );
                }
                $fts_share_svg = trim( $fts_share_svg );
                if ( $fts_share_svg !== '' && stripos( $fts_share_svg, '<svg' ) !== false ) {
                    $fts_share_svg = preg_replace(
                        '/<svg\\b(?![^>]*\\bclass=)([^>]*)>/i',
                        '<svg class="fts-v2-share-svg"$1>',
                        $fts_share_svg,
                        1
                    );
                    $fts_share_svg = preg_replace(
                    ...
                    ?>
                    <svg class="fts-v2-share-svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">
