...
class FTS_Destination_V2 {
    ...
    private static function get_destination_data( $term ) {
        // Make sure the price formatting filter respects the user-selected
        // currency on the very first page render (the plugin filter bails on
        // is_admin and we want a guaranteed override here).
        self::register_currency_override();
    ...
    public static function build_card_data( $id ) {
        ...
        // Always read raw price values from post meta. We avoid Trip::get_price()
        // because the plugin attaches conversion filters that behave differently
        // in admin/ajax contexts, producing inconsistent prices between the
        // initial page render and the Load More AJAX response.
        $price = 0; $sale = 0;
        if ( ! empty( $settings['trip_price'] ) ) {
            $price = floatval( $settings['trip_price'] );
        }
        if ( ! empty( $settings['trip_prev_price'] ) ) {
            $sale = floatval( $settings['trip_prev_price'] );
        }
        if ( ! $price ) {
        ...
        $price = self::maybe_convert_price( $price );
        $sale  = $has_sale ? self::maybe_convert_price( $sale ) : 0;
        ...
        if ( is_string( $trustindex ) && $trustindex !== '' ) {
            $trustindex = wp_kses_post( $trustindex );
        } else {
            $trustindex = '';
        }
    ...
    private static $active_currency_code = '';
    private static $currency_override_registered = false;
    ...
    private static function sync_currency_cookie() {
        ...
        } elseif ( ! empty( $_GET['wte_cc'] ) ) {
            $code = strtoupper( sanitize_text_field( wp_unslash( $_GET['wte_cc'] ) ) );
        }

        self::$active_currency_code = $code;

        self::register_currency_override();
    }

    /**
     * Register a high-priority filter that forces every call to
     * `wp_travel_engine_currency_code` to return the user-selected currency,
     * bypassing the plugin's `is_admin()` early-return.
     */
    private static function register_currency_override() {
        if ( self::$currency_override_registered ) {
            return;
        }
        $code = self::get_target_currency();
        if ( empty( $code ) ) {
            return;
        }
        self::$currency_override_registered = true;
        add_filter( 'wp_travel_engine_currency_code', function ( $current, $use_default ) use ( $code ) {
            if ( $use_default ) {
                return $current;
            }
            return $code;
    ...
    private static function get_target_currency() {
        if ( self::$active_currency_code ) {
            return self::$active_currency_code;
        }
        if ( ! empty( $_COOKIE['cc_code'] ) ) {
            return strtoupper( sanitize_text_field( wp_unslash( $_COOKIE['cc_code'] ) ) );
        }
        if ( ! empty( $_COOKIE['wte_currency_code'] ) ) {
            return strtoupper( sanitize_text_field( wp_unslash( $_COOKIE['wte_currency_code'] ) ) );
        }
        if ( ! empty( $_GET['wte_cc'] ) ) {
            return strtoupper( sanitize_text_field( wp_unslash( $_GET['wte_cc'] ) ) );
        }
        return '';
    }

    private static function maybe_convert_price( $price ) {
        ...
        $target = self::get_target_currency();
        if ( empty( $target ) ) {
            return $price;
        }
        ...
        try {
            ...
            // Primary: helper does API auto-update or manual rate based on settings.
            $converted = $helper->get_price( $base, $target, $price );

            // Fallback: when the helper fails (e.g. API returns false because the
            // remote service is down, the key is rate-limited, or the response is
            // invalid), it returns the original price unchanged. Detect that and
            // fall back to the manually-stored rates list (which the admin has
            // already configured), so prices still convert offline.
            if ( ! is_numeric( $converted ) || (float) $converted === (float) $price ) {
                $manual = self::get_manual_rate_for( $helper, $target );
                if ( $manual > 0 ) {
                    $converted = $price * $manual;
                }
            }

            return is_numeric( $converted ) ? round( (float) $converted ) : $price;
        } catch ( \Throwable $e ) {
            return $price;
        }
    }

    private static function get_manual_rate_for( $helper, $target_code ) {
        if ( ! method_exists( $helper, 'get_currency_converter_list' ) ) {
            return 0.0;
        }
        $list = $helper->get_currency_converter_list();
        ...
        foreach ( $list['code'] as $i => $code ) {
            if ( strtoupper( (string) $code ) === $target_code ) {
                $rate = isset( $list['rate'][ $i ] ) ? floatval( $list['rate'][ $i ] ) : 0.0;
    ...
    public static function format_price( $amount ) {
        ...
        $s = get_option( 'wp_travel_engine_settings', array() );
        $d = isset( $s['decimal_digits'] ) && $s['decimal_digits'] !== 'default' ? intval( $s['decimal_digits'] ) : 0;
        return '$' . number_format( floatval( $amount ), $d );
    }
}
